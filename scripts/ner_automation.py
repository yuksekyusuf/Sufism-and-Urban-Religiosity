...
#     resume_from_index=None  # Option to resume processing
# ):
#     """
#     Processes a DataFrame for NER with advanced batching and resume capabilities.
...
#         logger.info(f"Resuming processing from index {resume_from_index}")
#         df = df.loc[resume_from_index:]

#     # Batch processing with progress tracking
#     total_rows = len(df)
#     logger.info(f"Starting NER processing for {total_rows} rows")
...
#                     # Check for errors in NER result
...
#                             })
#                             batch_rows_to_write.append(person_row)
...
#                             })
#                             batch_rows_to_write.append(date_row)
...
#                                 'case_unique_id': case_id
#                             })
#                             batch_rows_to_write.append(place_row)
...
#                             'case_unique_id': case_id
#                         })
#                         batch_rows_to_write.append(legal_type_row)
...
#                             'case_unique_id': case_id
#                         })
#                         batch_rows_to_write.append(case_result_row)
...
#                             'case_unique_id': case_id
#                         })
#                         batch_rows_to_write.append(case_result_type_row)
#                     else:
#                         # Log errors for rows that failed NER processing
#                         logger.error(f"NER processing failed for row {index}: {ner_result.get('error', 'Unknown error')}")
    
#                 except Exception as row_error:
#                     logger.error(f"Unexpected error processing row {index}: {row_error}")
            
#             # Write batch results to CSV
#             if batch_rows_to_write:
#                 write_to_csv(
#                     output_file, 
#                     batch_rows_to_write, 
#                     headers=headers, 
...
#                 ner_results.extend(batch_rows_to_write)
            
#             # Progress logging
#             logger.info(f"Processed batch {batch_start//batch_size + 1}: "
#                         f"Rows {batch_start} to {batch_end}")
        
#         except Exception as batch_error:
#             logger.error(f"Error processing batch starting at {batch_start}: {batch_error}")
#             # Optionally, you can choose to break or continue
    
#     # Final logging
#     logger.info(f"Completed NER processing. Total rows processed: {total_rows}")
